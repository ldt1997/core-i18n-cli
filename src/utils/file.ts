 ...
 */
export function getCorei18nLocalLangPath(config: ProjectConfig) {
  const fileName = path.basename(
    config.localLangFile,
    path.extname(config.localLangFile)
  );
  return path.resolve(
    process.cwd(),
    `${config.corei18nDir}/${LOCAL_LANG_DIR}/${fileName}.js`
  );
}
 ...
 * @param config
 */
export async function getLocalLangMap(
  ...
  if (fs.existsSync(localLangFilePath)) {
    if (fileExtension === ".js") {
      const res = await import(localLangFilePath).then(
        (res) => res?.default?.default
      );
      return res;
    }
    ...
    if (fileExtension === ".ts") {
      ...
      if (fs.existsSync(corei18nLocalLangPath)) {
        const res = await import(corei18nLocalLangPath).then(
          (res) => res?.default?.default
        );
        return res;
      }
      const outDir = path.resolve(
        process.cwd(),
        `${config.corei18nDir}/${LOCAL_LANG_DIR}`
      );
