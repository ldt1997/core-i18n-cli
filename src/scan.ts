...
const resolveChineseWordsFromTSX = (path: string): Promise<string[]> =>
  new Promise((resolve) => {
    fs.readFile(path, "utf-8", (err, data) => {
      if (err) throw new Error("resolveChineseWordsFromTSX Error: " + err);
      ...
      traverse(ast, {
        enter(path) {
          if (
            ...
            return;
          if (isIntlDefaultMessage(path)) return;
...
async function generateIds(
  ...
  if (config.idType === "translate") {
    ...
    const promises = textChunks.map((item) =>
      translateTextByBaidu(item, config.baiduApiKey)
    );
    const transResults = await Promise.all(promises).then((results) =>
      results
        .map((res) =>
...
async function scan() {
  ...
  if (!!newWords.length) {
    ...
    for (let i = 0; i < newWords.length; i++) {
      ...
      if (!!config.idSuffix) id = config.idSuffix + "." + id;
      result[id] = word;
    }
