 ...
 * @param length
 */
export function generateHash(text, length = 16) {
...
export function isIntlDefaultMessage(path) {
    const ancestorIdentifier = path.findParent((p) => p.isCallExpression() &&
        // p?.node?.callee?.object?.name === "intl" &&
        p?.node?.callee?.property?.name === "formatMessage");
    const isIntlFormatMessage = ["defaultMessage", "description"].includes(path?.parent?.key?.name) &&
        !!ancestorIdentifier;
    const ancestorJSXIdentifier = path.findParent((p) => p.isJSXElement() &&
        p?.node?.openingElement?.name?.name === "FormattedMessage");
    const isFormatMessageJSX = ["defaultMessage", "description"].includes(path?.parent?.name?.name) &&
